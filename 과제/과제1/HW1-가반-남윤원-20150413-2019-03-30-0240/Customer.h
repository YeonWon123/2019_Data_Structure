...
public:
	Customer();		// 기본 생성자
	Customer(string m_name, int m_group_num, int m_turn, int m_arr_time);	// 생성자
	~Customer();	// 기본 소멸자
...
public:
	...
	// Postconditions : 이 함수가 종료되면, int값을 반환하므로 
	//                  = 우측에 있음으로서 int 변수에 값을 할당해줄 수 있다.
	//                  예를 들어, int d; d = getGroup_num(); 이라는 코드를 짰을 경우,
	...
	bool operator == (Customer& custom_a) const {
		...
		//					true 또는 false를 리턴한다.
		// return : (인자로 받은 클래스 변수 중 도착 시간)과 (현재 가진 클래스 변수 중 
	...
	bool operator < (Customer& custom_a) const {
		...
		//					다만, 호출된 객체와 현재 자신의 클래스 변수 중 도착 시간(클래스 안 멤버 변수)을 비교하여,
		//					true 또는 false를 리턴한다.		
		// return : (인자로 받은 클래스 변수 중 도착 시간)에 비해 (현재 가진 클래스 변수 중 
