...
{
	//cout << "first는 " << first << "이고, last는 " << last << "입니다!" << endl << endl;
	...
	for (i = first + 1; i <= last; i++) {
		...
		if (link_1->elem.getArr_time() <= link_2->elem.getArr_time()) {
			...
			for (int j = 0; j < lastSmall; j++)
				this->next(elem);
			link_2 = current;
			//cout << link_1->elem.getName() << "의 시간인 " << link_1->elem.getArr_time()
			//	<< "과" << link_2->elem.getName() << "의 시간인 " << link_2->elem.getArr_time() << "이 서로 바뀝니다!" << endl;
